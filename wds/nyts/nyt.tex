...
exists of course, especially for an area as uncertain as ecological
inference, but much can be done.  Our approach, {\bf in addition to
  the analysis of the bounds}, is to introduce formal Bayesian model
averaging procedures and to include a wide range of models that any
partisan might have considered.  The procedure then computes a
combined estimate from these models based on their relative
...
\emph{EI: A Program for Ecological Inference} (available at
...
\subsection{Statistical Analysis with a Single Model Specification} \label{s:onemodel}
...
each parameter of $\Theta$ as described in \citet{king:97}.  This
prior distribution and the likelihood function together define King's
model in a standard Bayesian framework.
