...
inclusion of covariates.  We rely on this point to produce a diverse
array of models to which we then apply Bayesian model averaging.
...
\caption{Estimated Bush's margin and Gore's probability of victory
  ...
  with selected other counterfactuals (each of which also includes
  excluding the invalid absentee ballots).  $^a$Corresponds to what
  would have happened if the U.S. Supreme Court had not stopped the
  manual recount.  $^b$Gore requested that Broward, Miami-Dade, Palm
  Beach, and Voulusia counties be recounted.  $^c$Only fully punched
  ballots were recounted in all counties.  $^d$Recounting all counties
  using the standard that any hanging chad or dimple was counted.
  $^e$A recount of the entire state, using the standards adopted by
  each county.} \label{tb:senario}
\end{center}
\end{table}
...
\multicolumn{4}{l}{\bf Counties not mentioned by the \emph{Times}$^b$} \\ 
...
\appendix
\section{Technical Issues} \label{appx:king}

King's (1997) ecological inference model begins by assuming that the
joint distribution of $\bg$ and $\bb$ are truncated bivariate normal
with mean, $\bbetau$, and a variance $\Sigmau$:
\begin{eqnarray}
...
simulations of $\bb$ and $\bg$ from their posterior above and
calculate simulations of $\Delta$.  By applying this procedure to each
of the 31 models, we get the first of the two components of Equation
...
approximation with posterior simulation. This estimator is known to
perform relatively well compared with other methods in many
situations.  The rate of approximation of the Laplace approximation is
$O(n^{-1})$, which is considerably better than easier-to-apply methods
such as BIC, which has a rate of only $O(n^{-1/2})$ (see e.g.
\citet{kass:tier:kada:89}).  For example, Kass and Raftery
(1995,p.778) say that ``even for very large samples, it [BIC] does not
produce the correct value.''  In our small-$n$ application, the
Laplace approximation that we use in combination with exact finite
sample posterior simulation is more
appropriate.\citep{raft:96,lewi:raft:97,dici:kass:raft:wass:97}.\footnote{If
  a model is estimated through a Markov chain Monte Carlo (MCMC)
...
% The Laplace-Metropolis estimator of the marginal likelihood is based
% on the following Laplace asymptotic approximation.
% \begin{equation}
