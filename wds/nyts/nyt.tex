...
\title{Did Illegally Counted Overseas Absentee Ballots Decide the 2000 U.S.\ 
...
\begin{abstract}
  ...
  litigation, butterfly ballot problems, recounts that might have been
  allowed but were not, or any other hypothetical divergence between
  voter preferences and counted votes.  After the election, the
  \emph{New York Times} conducted a six month long investigation and
  found that 680 of the overseas absentee ballots were illegally
  ...
  use these methods to make ecological inferences for other purposes.
  We also present a variety of new empirical results that delineate
  the precise conditions under which Al Gore would have been elected
  president, and offer new evidence of the striking effectiveness of
  the Republican effort to convince local election officials to count
...
ecological inference problem in general is often weak identification.
...
Bayesian model averaging estimate obtained in this fashion is known to
provides better in sample prediction, as mesured by a logarithmic
scoring rule, than using any single model \citep{madi:raft:94}. In our
application, this property of Bayesian model averaging is important
because our purpose is to predict the distribution of invalid ballots
in our data. On the other hand, if one is concerned with out of sample
prediction, similar methods such as Committee methods of neural
networks models are available \citep{bish:95}. Although the two
literatures rarely refer to each other, Bayesian model averaging and
Committee methods are closely related. Both are based on an idea of
obtaining one set of estimates from multiple models by averaging them
with weights. The only difference is the way weights are calculated.
For Committee methods, optimal weights minimize error correlation of
individual models such that out of sample prediction can be improved.
In contrast, Bayesian model averaging gives more weights to models
that are supported by data.\footnote{Another similar model is mixture
  models where weights are modeled parametrically (see e.g.
  \citet{rose:jian:tann:00,robe:96a}).}
...
we found for this quantity of $-126$ to 936.  The weight of the
...
is the lack of any noticable relationship between the degree to which
