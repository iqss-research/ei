...
\item[fit] Combination of xtfit and tomogP (and ztfit if applicable).

\item[fitT] Combination of xtfit and tomogT (and ztfit if applicable).
...
\item[zt] basic $Z_i$ by $T_i$ scatterplot. The same restriction as
\texttt{zt} applies. Available if $Z$ is the only covariate common for
\texttt{zb} and \texttt{zw}, or if $Z$ is the only covariate for
either \texttt{zb} or \texttt{zw} and no covariate for the other.
...
\item[ztC] basic $Z_i$ by $T_i$ scatterplot with circles sized
proportional to $\Npp$ or some other variable defined by the global
\texttt{\_eigraph\_circ}. The same restriction as \texttt{zt} applies.


\item[xtfit] $X_i$ by $T_i$ plot with estimated $\rE(T_i|X_i)$ and
  conditional 80\% confidence intervals.  See Figure 10.3, page 206.

\item[ztfit] $Z_i$ by $T_i$ plot with estimated $\rE(T_i|Z_i,
\rE(X|n))$ where $\rE(X|n)$ is the weighted average of $X$ and
conditional 80\% confidence intervals. The same restriction as
\texttt{zt} applies.
...
\item[\_eigraph\_Z] string: xlabel for zt plots. Default=\texttt{"Z"}

\item[\_eigraph\_Xhi] High end of X graphs. Default=1.

\item[\_eigraph\_Xlo] How end of X graphs. Default=0.

\item[\_eigraph\_Zhi] High end of Z graphs. Default=max(Z).

\item[\_eigraph\_Zlo] How end of X graphs. Default=min(Z).
...
\item[\_EIMetaR] If \texttt{dbuf} is a meta-data buffer (i.e., from
  ...
  accessed when running \texttt{eiread} and \texttt{eigraph}.
  Default=1.
  
\item[\_eiread_Zb] (rows(Zb) $\times$ cols(Zb)) matrix of new
  covariates for the calculation of first differences (default=\{.\};
  which indicates the use of the original covariate, \texttt{Zb}).
  
\item[\_eiread_Zw] (rows(Zw) $\times$ cols(Zw)) matrix of new
  covariates for the calculation of first differences (default=\{.\};
  which indicates the use of the original covariate, \texttt{Zw}).
  
\item[\_eiread_betabs] (p $\times$ \texttt{\_Esims}) new simulated
  draws of $\beta^b$ stored when the new covariates are specified
  using the globals, \texttt{\_eiread_Zb} and \texttt{\_eiread_Zw}
  (default=\{.\}; which indicates the use of the original simulated
  values of $\beta^b$). You need to clear this global variable by
  setting \texttt{\_eiread_betabs=\{.\}} or using \texttt{eiset;} when
  you use \texttt{eiread} with the new or original covariates.
...
\item[ExpVarCIZ] $100\times 4$: 80\% confidence intervals for $T_i$
given $Z_i$ and the weighted average of $X_i$.
Z$\sim$20\%CI$\sim$mean$\sim$80\%CI of sims from $P(T_i|Z)$, where $Z$
in this context is 100 numbers equally spaced between min(Z) and
max(Z). Available if $Z$ is the only covariate common for \texttt{zb}
and \texttt{zw}, or if $Z$ is the only covariate for either
\texttt{zb} or \texttt{zw} and no covariate for the other.  (Uses
...
\item[ExpVarCIsZ] $100\times 4$: same as expvarciz, but smoothed with
LOESS.  Used for eigraph's ztfit. \texttt{x2} simulations when used
with \texttt{ei2}).
...
\item[tsimsz] $100\times$\texttt{\_Esims}$+1$: simulations of $T_i$
given $Z_i$.  rows correspond to 100 values of $Z_i$ equally spaced
between min(Z) and max(Z), columns are $Z_i$ and sorted simulations of
$T_i|Z_i$. (Uses \texttt{x2} simulations when used with \texttt{ei2}).
