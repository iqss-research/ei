...
read using \texttt{v = eiread(dbufrun);} and \texttt{v =
eigraph(dbufrun);} while specifying the model number with the global
variable, \texttt{_EIMetaR} (default of this variable is 1 which reads
the results of \texttt{ei} run for Model 1).  
...
using \texttt{v = eiread(dbufavg);} and \texttt{v = eigraph(dbufavg);}
...
\texttt{dbufdef = eimodels\_def(ndbufdef,num,t,x,n,zb,zw);}, where
...
include a constant term).  \texttt{dbufdef} is an updated output meta
data buffer which contains the current model definition along with the
previously stored ones.

\texttt{dbufrun = eimodels\_run(dbufdef);}, where \texttt{dbufdef} is the
output meta data buffer from \texttt{eimodels\_def} which contains
multiple model definitions. \texttt{dbufrun} is the output meta data
