...
\item[\_EImarg] 1 if save (\_Esims$\times 1$) the likelihoods
  evaluated for each simulation in \texttt{marginal}; 0 saves only the
  means of these likelihoods (default).  These can be used for
...
\texttt{\_ei2\_r} to the number of the imputation data buffer you wish
to analyze.)
...
\item[\_Ei2\_r] If \texttt{dbuf} is a meta-data buffer (i.e., from the
...
\item{MargLik} If \texttt{\_EImarg}=1, this is \texttt{\_Esims}$\times
  1$ vector of marginal likelihood values; otherwise it is a scalar
  mean of these.
