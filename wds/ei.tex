...
\author{Gary King\\ Department of Government\\ Harvard
  University\thanks{\copyright\ Copyright 1995--2001, All rights
    reserved.  Littauer Center North Yard, Cambridge Massachusetts
    ...
    Copyright Aptech Systems, Inc., 1984-1998, and uses Gauss'
    Constrained Maximum Likelihood Module written by Ronald J.\
    ...
    procedure).  EI is copyrighted, but you may copy and distribute
...
If you have supervisor privileges, follow analogous steps as for DOS
...
\item The output data buffer from \texttt{ei} includes a large variety
  ...
  are treated the same.\footnote{Also stored in the output data buffer
    ...
    convenient if you are in the habit of following \emph{the
      replication standard} by making publicly available all the
...
procedures.  The procedure, \texttt{ndbuf1 =
  eimodels\_def(ndbuf1,num,t,x,n,zb,zw);}, enables you to define a
model specification (including all input variables
...
data buffer,'' \texttt{ndbuf1}.  The meta data buffer contains a set
...
times to store as many models in \texttt{ndbuf1} as needed (the first
time you run the procedure, set \texttt{ndbuf1="";}).

Once all model specifications have each been defined and stored, use
\texttt{ndbuf2 = eimodels\_run(ndbuf1);} to run \texttt{ei())} on each
of the stored specifications and save the results in another meta data
buffer, \texttt{ndbuf2}.  Finally, run \texttt{dbuf =
  eimodels\_avg(ndbuf1);} to use Bayesian Model Averaging to combine
the results from multiple models with weights (the procedure
calculates) indicating how much the data supports each model.  The
results of this model averaging can be read using \texttt{v =
  eiread(dbuf);} where \texttt{dbuf} is the output data buffer. To use
this method, the prior distribution for covariates should be specified
for each model using the global variables, \texttt{\_EalphaB} and
\texttt{\_EalphaB}, so that the resulting posterior distribution is
known to be proper.
...
\item[\_Ecdfbvn] Determines which procedure to use for computing the
  ...
  inaccurate and jump discontinuously and for very small values.
...
\item[\_Eisn] factor to multiply by \texttt{\_Esims} to compute the
  number of normals to draw before resampling.  This is used to try
...
of commands that you might run interactively on a DOS-based machine.
...
\texttt{ndbuf1} is an input meta data buffer which contains all the
model specifications previously defined as separate EI data buffers
(or ``'' for the first run), and to which the current model definition
will be added, \texttt{num} is the scalar integer model number to be
assigned to the current model definition, and the rest are defined as
in \texttt{ei}: \texttt{t}, \texttt{x}, and \texttt{n} are $p\times 1$
vectors, \texttt{zb} is a scalar 1 for no covariates or a $p\times
k^b$ matrix of covariates, and \texttt{zw} is the scalar 1 for no
covariates or a $p\times k^w$ matrix of covariates (do not include a
constant term).  \texttt{ndbuf2} is an output meta data buffer which
contains the current model definition along with the previously stored
ones.
...
multiple model definitions. \texttt{ndbres} is the output meta data
buffer, containing one data buffer for each data buffer of definitions
in \texttt{ndbuf2}.

\texttt{dbres = eimodels\_avg(ndbres);}, where \texttt{ndbres} is the
output meta data buffer from \texttt{eimodels\_run} containing the
results of \texttt{ei} run for multiple models. \texttt{dbres} is a
...
buffer should be read with \texttt{eiread}.
...
ndbuf = eimodels_def("",1,t,x,n,1,1);    @ save Model 1 @
...
_EIMetaR = 2;
call eiread(ndbres, "sum");              @ summary info for Model 2 @
...
call eiread(dbres,"sum");                @ summary for dbres @
...
\item[MargLLik] ($\#$ of models)$\times 2$: the first column is the
  model number and the second column indicates the marginal
  log-likelihood estimate of the specified model. It is used to
  calculate the posterior model probability as a part of the Bayesian
  Model Averaging procedure (can be read only from the output buffer
  of \texttt{eimodels\_avg}).

\item[MeanIR] scalar log of the mean importance ratio.
  
\item[Mfreq] ($\#$ of models)$\times 2$: the first column is the model
  number and the second column indicates how many posterior draws are
  taken from the specified model. (can be read only from the output
  buffer of \texttt{eimodels\_avg}).
...
\item[PostProb] ($\#$ of models)$\times 2$: the first column is the model
  number and the second column indicates to the posterior model
  probability for each model as a result of the Bayesian Model
  Averaging (can be read only from the output buffer of
  \texttt{eimodels\_avg}).
  
\item[Pphi] $2\times 5$ maximum posterior estimates.  row 1: $\phi$,
  row 2: standard errors.  See Chapter 7.

\item[PrProb] ($\#$ of models)$\times 2$: the first column is the
  model number and the second column indicates to the prior model
  probability for each model as an input of the Bayesian Model
  Averaging specified with the global variable,
  \texttt{\_EI\_bma\_prior} (can be read only from the output buffer
    of \texttt{eimodels\_avg}).
...
are continuous but restricted to the unit interval (such as the
...
court in \emph{Bill Jones vs.  Ca Democratic Party}.  In each case, EI
was unchallenged by experts on the opposing side.  Since EI starts
with Goodman's regression, which had previously been declared as the
appropriate method by the Supreme Court, and adds information known to
be true (i.e., the bounds), EI has been relatively uncontroversial in
these situations.
...
globals to
\hlink{\texttt{KBenoit@Harvard.Edu}}{mailto:kbenoit@harvard.edu} if
