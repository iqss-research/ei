...
framework.  The problems with HS's approach are due to assuming
linearity when modeling an inherently nonlinear and bounded
...
$\hat\beta_i^b$ on $Z_i$ and every other procedure discussed in HS's
paper and the literature we have examined.  This is true even if we
knew the true value of the adjustment.  Unless $Z$ has a zero mean,
the regression line from HS's method tends to miss the cloud of true
...
method naturally extends HS's framework and dominates HS's procedure.
We also compare our fully adjusted procedure to unadjusted linear
...
never called for.  Overall, when the bounds are narrow, the fully
adjusted procedure can be computed, but the adjustment makes little
...
HS's simulation technique but not any real application.
...
intervals, which were not available in HS's version.\footnote{To
  ...
  run EI on $X$ and $T$; (2) regress $\hat\beta_i^b$ on $Z_i$ yielding
  the estimated intercept $\hat\alpha_U$ and slope $\hat\gamma_U$; (3)
  ...
  ($\hat\beta_i^b,\hat\beta_i^w$) for all $i$; (7) regress
  ($\hat\beta_i^b-\beta_i^b$) on $\beta_i^b$ to estimate $\delta_0^b$
  and $\delta_1^b$; (8) draw $\tilde\alpha_U$ and
  $\tilde\gamma_U$ from the posterior provided by LS; (9) compute
  the adjusted intercept as
  $(\tilde\alpha_U-\hat\delta_0^b)/(1+\hat\delta_1^b)$ and
  adjusted slope as $\tilde\gamma_U/(1+\hat\delta_1^b)$; (10)
...
to $\beta_i^b$ plus random noise.  Unfortunately, this induces immense
attenuation bias due to random measurement error in the explanatory
variable, quite apart from any attenuation bias that may occur due to
the Bayesian shrinkage in the EI estimate.
...
Finally, HS' Monte Carlo simulations do not test the actual adjustment
procedure they propose.  All the results they present in their Section 6
rely on the true $\beta_i^b$'s to estimate $\delta_0^b$ and
$\delta_1^b$, though this is the one quantity users of second-stage
regressions are sure to lack.  Appropriately, HS recommend a different
estimation procedure (in their Section 7.2) for use when $\beta_i^b$
is unknown, but leave this procedure untested.  Hence there is no
direct evidence that adjustment would be less biased or more efficient
than unadjusted least squares in practice.
...
would make for an improvement over least squares on the estimated
$\hat\beta_i^b$'s.  Of course, the true adjustment is not known and
needs to be estimated.  Unfortunately, it cannot.  The reason is that
EI models $\beta_i^b$ as a random effect constrained to be within the
precinct-level bounds.  If $Z_i$ is not in the EI first stage (which
is true by definition, since if it were included we wouldn't need a
second stage), then the \emph{only} information in $\hat\beta_i^b$
that could be predicted by $Z_i$ would come from the bounds.  The same
is true of the adjustment procedure: the \emph{only} information with
which to estimate $\delta_0^b$ and $\delta_1^b$ comes from the bounds.  If
the bounds are relatively uninformative, as we are assuming in this
first prototypical case, then there is little information with which
to estimate the adjustment.  On reflection, this should not be a
...
the data for this figure from the same model as Figure \ref{f:narrow},
...
corrects for most of the attenuation.  The fully adjusted (FA) line,
in contrast, over-corrects for attenuation.
\begin{figure}[t]
  \begin{center}
...
on which HS focus the most, and find that none of the proposed
corrections affect the substantive conclusions of the original study.
Table \ref{t:bkrep} presents these results (assuming arguendo that
other features of their model are correctly specified).
\begin{table}[th]
\label{t:bkrep}
\begin{center}
