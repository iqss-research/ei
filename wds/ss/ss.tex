...
and bounded relationship, and HS's decision to adjust the slop without
...
HS suggest that researchers who wish to estimate a linear second-stage
regression using EI estimates
\begin{equation}
\label{eq:ssreg}
\hat\beta_i^b=\alpha_U+\gamma_U Z_i + \nu_i,
\end{equation}
make an adjustment to the slope $\gamma$, but make no suggestion
regarding the intercept $\alpha$.  However, the logic underlying HS's
slope adjustment requires simultaneous adjustment of the constant.
Specifically, HS assume that the measurement error in EI estimates,
$\hat\beta_i^b - \beta_i^b$, can be approximated by the linear
regression
\begin{equation}
\label{eq:hserr}
\hat\beta_i^b - \beta_i^b = \delta_0^b + \delta_1^b\beta_i^b + \zeta_i.
\end{equation}
If this holds, simple algebraic substitution into Eq. \ref{eq:ssreg}
shows that
\begin{equation}
\label{eq:hscorr}
\beta_i^b = \frac{\alpha_U-\delta_0^b}{1+\delta_1^b} +
\frac{\gamma_U}{1+\delta_1^b}Z_i.
\end{equation}
The slope in this equation provides the HS slope correction.  Clearly,
this adjusted slope only makes sense in the context of Eq.
\ref{eq:hscorr}.  Thus users of HS slope correction should also
adjuste the constant, which is easily accomplished by calculating
$\alpha_A = (\alpha_U-\delta_0^b)/(1+\delta_1^b)$.  Failure to adjust
all parameters often leads to a worse fit than the unadjusted
regression, and the fully adjusted procedure generally dominates slope
correction alone.
\begin{table}[tb]
\label{t:hsrep}
\begin{center}
...
(marked WLS) line is almost on top of the LS line.  In contrast, the
line representing the fully adjusted method we developed (marked FA)
fits the true points well, and hence corrects for the attenuation.
Figure \ref{f:wide} also plots Herron and Shotts' partially adjusted
method (marked HS), which as is more biased than any of the
...
\begin{figure}[t]
  \begin{center}
    ...
    \caption{Data with Informative Bounds. Plot of the $Z_i$ horizontally by
      ...
      how all three methods give almost the same answer.}
    \label{f:narrow}
  \end{center}
\end{figure}
...
\begin{figure}[t]
  \begin{center}
    ...
    \caption{Data with Narrow and Wide Bounds. Plot of the $Z_i$ 
      ...
      for most of the attenuation bias.}
    \label{f:mixed}
  \end{center}
\end{figure}
...
\begin{figure}[t]
  \begin{center}
    ...
    \caption{Data with a Nonlinear Relationship.  Plot of $Z_i$ 
      ...
      the linear methods give out of bounds predictions.}
    \label{f:nonlinear}
  \end{center}
\end{figure}
...
(Beck, 1997).  They found that ticket splitting rises with the
...
interchangeably).  All three methods support Burden and Kimball's
argument that ticket-splitting results not from intentional balancing,
but as an unintential result of campaign specific factors.

Note that Table \ref{t:bkrep} reports no standard errors for the fully
adjusted method; HS offer no procedure for computing standard errors
of their partially adjusted method and we have not bothered to finish
this aspect of their work.  It would be possible (by extending beyond
linear econometrics), although slightly complicated because the
adjusted estimators are ratios and also because of dependence of the
...
Form this figure, we can then see all the data, precisely how
