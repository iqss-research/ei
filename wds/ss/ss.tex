...
intervals, which were not available in HS's version.\footnote{To
  ...
  and $\delta_1^b$; (8) draw $\tilde\alpha_U$ and $\tilde\gamma_U$
  from the posterior provided by LS; (9) compute the adjusted
  intercept as $(\tilde\alpha_U-\hat\delta_0^b)/(1+\hat\delta_1^b)$
  and adjusted slope as $\tilde\gamma_U/(1+\hat\delta_1^b)$; (10)
...
second-stage regression in Herron and Schott's Monte Carlo 
...
the intercept and slope coefficients accurately, effectively
correcting the problem with the HS's procedure.
...
Since HS's proposal is never better than full adjustment, and often
dramatically worse, we do not consider it further.
\begin{figure}[t]
  \begin{center}
...
interchangeably).  We also computed standard errors for the fully
adjusted method and found that they were an average of *** times
larger than that for LS and WLS, which is an indication of the
inefficiency of adjusted method.  Thus, in this example, any the minor
reduction in bias that may occur is outweighed by the substantial
increase in variance.  A mean square error criterion, which combines
bias and variance, would suggest not making the adjustment even in the
presence of attenuation bias.  (We confirmed that this result was
fairly general with Monte Carlo simulations, which we also do not
present here.)  This is an intuitive result given that the bias
adjustment takes the form of a ratio, and both the numerator (which is
LS) and denominator contain estimation uncertainty.  It is also
consistent with the experience of many others trying to create bias
adjustments in a wide variety of models outside of the field of
