...
\author{Gary King\thanks{Alfred J. Weatherhead III University
    Professor, Institute for Quantitative Social Science, 1737
    Cambridge Street, Harvard University, Cambridge MA 02138;
    \url{http://GKing.harvard.edu}, king@harvard.edu, (617) 500-7570.}
  \and Benjamin Schneer\thanks{bschneer@fas.harvard.edu} }
...
whether the proposed map has a retrogressive effect.  Our analysis is
based on the quantitative information; much supplementary qualitative
...
behavior is unavailable because of the secret ballot. Thus, modern
methods of ecological inference are used to estimate (rather than
determine) individual voting behavior based on publicly available
VTD-level (precinct) aggregated votes for each of the candidates based
on election results as well as VTD-level voting age population from
the US Census.  We summarize here the methods of ecological inference
we use.
...
(100\% black, 100\% white, or 100\% Hispanic) because for those
precincts we know for certain the voting behavior of an ethnic group.
...
approach was introduced. King's approach was the first to combine the
...
other deterministic information known for certain from other
precincts. Impossible estimates are never produced by this
methodology, and all information from all precincts are used in the
analysis. Like any indirect method of revealing information that the
secret ballot hides, EI is also uncertain to a degree, but it uses
more information than any other approach. Since 1997, a variety of
other methods have been developed in the academic literature by King
and others, virtually all of which now follow King's practice of
including deterministic and statistical information in the same model.
For example, \citet{king2001} extends King's method to arbitrarily
large numbers of ethnic groups and candidates; we use this method in
our work as well as King's original method. Some of these other
methods have been collected in the edited volume by \citet{king2004}.
...
fairly inaccurate most of the time. Studies have shown that
uncertainty remains with King's and subsequent methods, but the
estimates are normally superior. Differences among the methods that do
...
that votes for each of the candidates. We also use King's “tomography
plots” and other diagnostic methods that help us discern when
adjustments in the methods need to be made.  And finally, we have used
the results from the ecological inference analysis to measure what we
call district performance, which is the percent of the voting age
population which a given ethnic group needs in order to receive a
specified share of the vote -- in this case we use 50 or 55 percent as
the specified thresholds. We do this under the assumption that voters
in a racial group in the district will have the same voting behavior
as voters in a racial group from adjacent districts.
...
\input{tables/smine_cvap_cd_3}
\input{tables/smine_cvap_cd_7}

\input{tables/pres04_cvap_cd_3}
\input{tables/pres04_cvap_cd_7}

\input{tables/pres08_cvap_cd_3}
\input{tables/pres08_cvap_cd_7}

\input{tables/sos06_cvap_cd_3}
\input{tables/sos06_cvap_cd_7}

\input{tables/sos10_cvap_cd_3}
\input{tables/sos10_cvap_cd_7}

\subsection{Benchmark Map}
\clearpage

\input{tables/smine_cvap_cd_4_benchmark}
\input{tables/smine_cvap_cd_7_benchmark}

\input{tables/pres04_cvap_cd_4_benchmark}
\input{tables/pres04_cvap_cd_7_benchmark}

\input{tables/pres08_cvap_cd_4_benchmark}
\input{tables/pres08_cvap_cd_7_benchmark}

\input{tables/sos06_cvap_cd_4_benchmark}
\input{tables/sos06_cvap_cd_7_benchmark}

\input{tables/sos10_cvap_cd_4_benchmark}
\input{tables/sos10_cvap_cd_7_benchmark}
...
%\input{tables/smine_vap_cd_3}
%\input{tables/smine_vap_cd_7}

%\input{tables/pres04_vap_cd_3}
%\input{tables/pres04_vap_cd_7}

%\input{tables/pres08_vap_cd_3}
%\input{tables/pres08_vap_cd_7}

%\input{tables/sos06_vap_cd_3}
%\input{tables/sos06_vap_cd_7}

%\input{tables/sos10_vap_cd_3}
%\input{tables/sos10_vap_cd_7}

%\subsection{Benchmark Map}
%\clearpage
%\input{tables/smine_vap_cd_4_benchmark}
%\input{tables/smine_vap_cd_7_benchmark}

%\input{tables/pres04_vap_cd_4_benchmark}
%\input{tables/pres04_vap_cd_7_benchmark}

%\input{tables/pres08_vap_cd_4_benchmark}
%\input{tables/pres08_vap_cd_7_benchmark}

%\input{tables/sos06_vap_cd_4_benchmark}
%\input{tables/sos06_vap_cd_7_benchmark}

%\input{tables/sos10_vap_cd_4_benchmark}
%\input{tables/sos10_vap_cd_7_benchmark}

\section{CVAP LD}
\subsection{Proposed Map}
\clearpage

\input{tables/smine_cvap_ld_2}
\input{tables/smine_cvap_ld_3}
\input{tables/smine_cvap_ld_4}
\input{tables/smine_cvap_ld_7}
\input{tables/smine_cvap_ld_8}
\input{tables/smine_cvap_ld_19}
\input{tables/smine_cvap_ld_24}
\input{tables/smine_cvap_ld_26}
\input{tables/smine_cvap_ld_27}
\input{tables/smine_cvap_ld_29}
\input{tables/smine_cvap_ld_30}

\subsection{Benchmark Map}
\clearpage
\input{tables/smine_cvap_ld_2_benchmark}
\input{tables/smine_cvap_ld_13_benchmark}
\input{tables/smine_cvap_ld_14_benchmark}
\input{tables/smine_cvap_ld_15_benchmark}
\input{tables/smine_cvap_ld_16_benchmark}
\input{tables/smine_cvap_ld_23_benchmark}
\input{tables/smine_cvap_ld_24_benchmark}
\input{tables/smine_cvap_ld_25_benchmark}
...
%\input{tables/smine_vap_ld_2}
%\input{tables/smine_vap_ld_3}
%\input{tables/smine_vap_ld_4}
%\input{tables/smine_vap_ld_7}
%\input{tables/smine_vap_ld_8}
%\input{tables/smine_vap_ld_19}
%\input{tables/smine_vap_ld_24}
%\input{tables/smine_vap_ld_26}
%\input{tables/smine_vap_ld_27}
%\input{tables/smine_vap_ld_29}
%\input{tables/smine_vap_ld_30}

%\subsection{Benchmark Map}
%\clearpage

%\input{tables/smine_vap_ld_2_benchmark}
%\input{tables/smine_vap_ld_13_benchmark}
%\input{tables/smine_vap_ld_14_benchmark}
%\input{tables/smine_vap_ld_15_benchmark}
%\input{tables/smine_vap_ld_16_benchmark}
%\input{tables/smine_vap_ld_23_benchmark}
%\input{tables/smine_vap_ld_24_benchmark}
%\input{tables/smine_vap_ld_25_benchmark}
%\input{tables/smine_vap_ld_27_benchmark}
%\input{tables/smine_vap_ld_29_benchmark}
