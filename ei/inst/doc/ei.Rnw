...
\item{{\verb#ei#}: To run the main procedure}
  ...
  Use the format, \verb#dbuf = ei(t,x,n)#, which takes three $
...
\noindent When we include the data argument, the arguments for \verb#t,x,n# should be strings rather than vectors. To include a covariate on $\beta_i^b$ simply specify a covariate vector for the option \verb#Zb#, or a string for the name of the covariate in the dataset.  Similarly, to include a covariate on $\beta_i^w$, specify a covariate vector for for the option \verb#Zw#, or a string for the name of the covariate in the dataset.\\
\newline
