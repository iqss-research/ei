...
example of \verb#EI#, refer to the user's guide below.)  As only four
...
\item{{\verb#ei#}: To run the main procedure}
  ...
  Use the format, \verb#dbuf = ei(t,x,n,1,1)#, which takes three $
  ...
  (e.g. the total number of people in the voting age population). The
  remaining two inputs are for optional covariates.  For the basic
  model, set them each to 1 for no covariates. The output of this
  procedure is the list \verb#dbuf# (i.e. an output object of class
  list called a ``data buffer''). After running \verb#ei#, it is a
  good idea to save \verb#dbuf# on disk for further analysis.  The
...
\item{{\verb#summary#}: To obtain a general summary of district-level information}
  ...
  \verb#ei# results.  It also includes information on the values
...
\item{{\verb#plot#}: To graph relevant information}
  ...
  the list that is the output of \verb#ei#, and name can be any number
  of a long list of ready-made graphs.  For example,
  \verb#plot(dbuf,"tomog")#s will plot a tomography graph, and
...
distribution based on the inputed data. Then it estimates county-level
