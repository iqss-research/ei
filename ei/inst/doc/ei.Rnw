...
While eiRxC models are helpful, the uncertainty estimates they produce are conditional on the model assumed. The key problem with ei models is not the sampling uncertainty in the statistical model, but the uncertainty associated with choosing a model, or model dependence. A tomography plot that contains information about the possible values of the quantities of interest is most useful in understanding the true amount of information in the data from which the model is obtaining its estimates. 
...
\begin{figure}[Ht]
\begin{center}
\includegraphics[width=2.75in, height=2.75in, viewport=0 0 420 395,clip]{whiteblackRxC} 
\end{center}
\caption{Comparing Estimates to the Truth at the County Level}
\label{fig:truth}
\end{figure}

Each of the polygons represented on this plot represent the area of possibilities for a given precinct for percent white turnout and percent black turnout. The plot follows what we call the Law of Conservation of Ink, where tomography polygons with more information (smaller polygons) are colored brighter than those with less information (larger polygons).  The colors on the plot are a heat map that are brighter when polygons overlap each other.This polygons shows that there is a lot of information about the percent white turnout, as the vast majority of the polygons are in the low area of percent white turnout, but very little information about the percent black turnout, as most of the polygons seem to cover the entire range possible for percent black turnout.
