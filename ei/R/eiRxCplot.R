...
eiRxCplot <- function(ei.object, random =FALSE, black=FALSE, hispanic=FALSE,white=FALSE,informative=FALSE, title,xaxis, yaxis, percent, data){
...
ok2 <- NULL
 rand <- rep(TRUE, length(x[ok]))
  if (random ==TRUE) {
       rand <- sample(c(TRUE, FALSE),length(x[ok]),replace=T, prob=c(percent,1-percent))
}
  if (black == TRUE) {
    bl <- data$black.ei[ok] >.65}
  if (hispanic==TRUE){
 his <- data$hisp.ei[ok] > .65}
  if (white==TRUE){
 whit <- data$whit.ei[ok] > .65
 }
 none <- !bl & !his & !whit
#print(sum(ok2))
  if (informative==TRUE) {
