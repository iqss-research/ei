...
tomogRxC3d <- function(formula, data, total = NULL, lci = TRUE, estimates = FALSE,
  ...
  for (i in 1:dim(hstr)[1]) {
    ...
    for (j in 1:length(xaxs)) {
      if (xaxs[j] < 1e-5) {
        xaxs[j] <- 0
      }
    }
    for (k in 1:length(yaxs)) {
      if (yaxs[k] < 1e-5) {
        yaxs[k] <- 0
      }
    }
    for (l in 1:length(xaxs)) {
      if (xaxs[l] > .99999) {
        xaxs[l] <- 1
      }
    }
    for (m in 1:length(yaxs)) {
      if (yaxs[m] > .99999) {
        yaxs[m] <- 1
      }
    }
    ...
    coords.axs <- if (length(coords.axs[1, ]) != 10) {
      cbind(coords.axs, matrix(data = NA, nrow = 2, ncol = (10 - length(coords.axs[1, ]))))
    } else {
      coords.axs
    }
  ...
  for (i in 1:dim(hstr)[1]) {
    ...
    for (j in 1:nrow(coords)) {
      ...
      if (coords[j, 3] < 1e-5 & is.na(coords[j, 3]) == F) {
        coords[j, 3] <- 0
      }
      if (coords[j, 3] > .99999 & is.na(coords[j, 3]) == F) {
        coords[j, 3] <- 1
      }
  ...
  for (i in 1:nrow(w)) {
    ...
    if (length(unique(w[i, ])) == 1 | length(unique(b[i, ])) == 1 |
      ...
      if (length(na.omit(b[i, ])) > 2) {
        area[i] <- (sqrt(abs(h[i, 1] - h[i, 2])^2 + abs(w[i, 1] - w[i, 2])^2)) * .01
      }
      if (length(na.omit(h[i, ])) > 2) {
        area[i] <- (sqrt(abs(w[i, 1] - w[i, 2])^2 + abs(b[i, 1] - b[i, 2])^2)) * .01
      }
    }
    # All other Planes
  ...
  if (ci == T) {
    ...
    rgl::points3d(mean(means.b), mean(means.w), mean(means.h),
      col = "black", size = 10, add = T
    ) # Mean of average simulations across precincts
