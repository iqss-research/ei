/*
** A support procedure for stwtotal.prg
** 
** This procedure prints out the state-level
** aggregate results from district level results
** of statewide primary races.
**
*/


proc (0)=stwprint(ndbuf);
 local p,c,dbuf1,dbuf2,cty,n,n2,nb,t,t2,x,x2,nt,v,d,dv,betab,betaw,cbetab,
  ...
  ?;
  "Bounds on composition of electorate";
  "        HISPAN  BLACK   WHITES";
  "Lower:" abds1[1,1]*sumc(nb[.,1])/sumc(nt)~abds1[1,5]*sumc(nb[.,3])/sumc(nt)~abds1[1,3]*sumc(nb[.,2])/sumc(nt);
  "Upper:" abds1[2,1]*sumc(nb[.,1])/sumc(nt)~abds1[2,5]*sumc(nb[.,3])/sumc(nt)~abds1[2,3]*sumc(nb[.,2])/sumc(nt);
  ?;
