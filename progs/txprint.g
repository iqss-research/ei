...
** 
   txprint(ndbuf); 
**
** This proc prints out the EI and EI2 results for Hispanics, Blacks and 
** Whites+Others. The results are based on the two EI/EI2 runs which are 
...
**
** INPUT
**   ndbuf = a nested data buffer which includes two data buffers, 
**           dbuf1 (EI for Hispanic vs. Non-Hispanics) 
**           dbuf2 (EI for Whites+Others vs. Hispanics+Blacks). 
**           dbuf3 (EI2 for Hispanic vs. Non-Hispanics) 
**           dbuf4 (EI2 for Whites+Others vs. Hispanics+Blacks). 
**
**           dbuf1 also has to have the two variables: 
**           "county" (county number) and "cname" ("county name").
**
** OUTPUT
...
proc (0)=txprint(ndbuf);
 local p,c,dbuf1,dbuf2,cty,n,n2,nb,t,t2,x,x2,nt,v,d,dv,betab,betaw,cbetab,
  ...
  n2 = eiread(vread(ndbuf,"dbuf2"),"_n");
  t = eiread(vread(ndbuf,"dbuf1"),"t");
  t2 = eiread(vread(ndbuf,"dbuf2"),"_t"); 
  nt = eiread(vread(ndbuf,"dbuf1"), "Nt");
  v = eiread(vread(ndbuf,"dbuf2"),"t");
  dv = nt.*v;

  /* loop for Hispanic and Whites */
  for i (1,2,1);
    dbuf1=vread(ndbuf,"dbuf"$+ftos(i*2-1,"*.*lf",1,0));
    dbuf2=vread(ndbuf,"dbuf"$+ftos(i*2,"*.*lf",1,0));
    ...
    /*** lambdas ***/
    lambdab[.,i] = eiread(dbuf2, "betab");
    lambdaw[.,i] = eiread(dbuf2, "betaw");
    {vx, clambdab[.,i], nx} = cmeanwc(lambdab[.,i], cty, nb[.,i]);  
    bnds = eiread(dbuf2, "bounds2");
    {vx, lbnds1[.,i], nx}= cmeanwc(bnds[.,1], cty, nb[.,i]);
    {vx, lbnds2[.,i], nx}= cmeanwc(bnds[.,2], cty, nb[.,i]);
    paggs2[.,(i*2-1):i*2] = eiread(dbuf2, "paggs");
    abds2[.,(i*2-1):i*2] = eiread(dbuf2, "abounds2");
    double[.,i] = eiread(dbuf2, "double");
  endfor;
  ...
  /*** lambdas for blacks ***/
  {temp,temp1} = bounds2(v,t,x[.,3],n);
  bnds = temp[.,1:2];
  {nx, lbnds1[.,3], nx}= cmeanwc(bnds[.,1], cty, nb[.,3]);
  {nx, lbnds2[.,3], nx}= cmeanwc(bnds[.,2], cty, nb[.,3]);
  abds2[.,5:6]=temp1';
  
  x2[.,3] = 1-x2[.,1]-x2[.,2];
  lambdabs = (v-eiread(vread(ndbuf,"dbuf2"),"betabs").*x2[.,1]
            -eiread(vread(ndbuf,"dbuf4"),"betabs").*(1-x2[.,2]-x2[.,3]))./x2[.,3];
  
  for i (1,p,1);
    temp = delif(lambdabs[i,.]',(((lambdabs[i,.]'.<bnds[i,1])+(lambdabs[i,.]'.>bnds[i,2])
           +scalmiss(bnds[i,1])+scalmiss(bnds[i,2])).>0));
    lambdab[i,3]=meanc(temp);
    if rows(temp)/=100 and not(scalmiss(temp));
      temp = rndsmpl(temp,100);
      lambdabs[i,.]=temp';
    endif;
    if scalmiss(temp);
      for j (1,100,1);
        lambdabs[i,j]=miss(0,0);
      endfor;
    endif;
  endfor;
  
  for i (1,rows(unicty),1);
    if scalmiss(lbnds1[i,3]) or sumc(selif(nb[.,3],cty.==unicty[i]))/sumc(selif(n,cty.==unicty[i]))<0.005;
      clambdab[i,3]=miss(0,0);
    else;
      clambdab[i,3]= (sumc(selif(nt,cty.==unicty[i]))/sumc(selif(n,cty.==unicty[i]))
                  *sumc(selif(dv,cty.==unicty[i]))/sumc(selif(nt,cty.==unicty[i]))
                  -clambdab[i,1]*cbetab[i,1]*sumc(selif(nb[.,1],cty.==unicty[i]))/sumc(selif(n,cty.==unicty[i]))
                  -clambdab[i,2]*cbetab[i,2]*sumc(selif(nb[.,2],cty.==unicty[i]))/sumc(selif(n,cty.==unicty[i])))
                  /(cbetab[i,3]*sumc(selif(nb[.,3],cty.==unicty[i]))/sumc(selif(n,cty.==unicty[i])));
      clambdab[i,3];
      if (clambdab[i,3]>lbnds2[i,3] or clambdab[i,3]<lbnds1[i,3]);
        clambdab[i,3] = miss(0,0);
      endif;
    endif;
  endfor;
  
  paggs2[1,5] = (sumc(nt)/sumc(n)*sumc(dv)/sumc(nt)-paggs1[1,1]*paggs2[1,1]*sumc(nb[.,1])/sumc(n)
                -paggs1[1,3]*paggs2[1,3]*sumc(nb[.,2])/sumc(n))/(paggs1[1,5]*sumc(nb[.,3])/sumc(n));                
  paggs2[2,5] = sqrt(diag(vcx(meanwc(lambdabs,nb[.,3]))));

  if (paggs2[1,5]>abds2[2,5]) or (paggs2[1,5] < abds2[1,5]);
    paggs2[1,5]=miss(0,0);
    paggs2[2,5]=miss(0,0);
  endif;
  if paggs2[1,5]==miss(0,0);
    paggs2[2,5]=miss(0,0);
  endif;
  if paggs1[1,5]==miss(0,0);
    paggs1[2,5]=miss(0,0);
  endif;  
  {a,temp,temp,temp,temp,temp}=reg(x[.,3]~(1-x[.,3]),t);
  {b,temp,temp,temp,temp,temp}=reg(x[.,3]~(1-x[.,3]),v.*t);
  double[.,3] = b./a;

  if rows(unicty)==1;
    paggs1[1,1 3 5]=cbetab;
    paggs2[1,1 3 5]=clambdab;
  endif;
  ...
  ?;
  format/rd 10,0;
  print "Democratic Votes:" sumc(dv);
  format/rd 7,1;
  print "Percent Democrat:" sumc(dv)/sumc(nt)*100;
  ?;
  "******************************************";
  ?;
  ...
  ?;
  "Estimates of Aggregate Quantities of Interest";
  "  LAMB-H  LAMB-B  LAMB-W";;
  paggs2[.,1 5]~paggs2[.,3];
  ?;
  "Aggregate Bounds";
  "        LAMB-H  LAMB-B  LAMB-W";
  "Lower:" abds2[1,1 5]~abds2[1,3];
  "Upper:" abds2[2,1 5]~abds2[2,3];
  ?;
  "cnty#   County  lambh lambb lambw lh-lo lh-up lb-lo lb-up lw-lo lw-up";;
  ?; 
  fmt = {"*.*lf" 5 0, "*.*lG" 10 10, "*.*lf" 6 3, "*.*lf" 6 3, 
  "*.*lf" 6 3, "*.*lf" 6 3, "*.*lf" 6 3, "*.*lf" 6 3, "*.*lf" 6 3, 
  "*.*lf" 6 3, "*.*lf" 6 3};
  for k (1,rows(vx),1);
    temp = vx[k]~ctynm[vx[k]]~clambdab[k,1 3]~clambdab[k,2]~lbnds1[k,1]~lbnds2[k,1]~ 
           lbnds1[k,3]~lbnds2[k,3]~lbnds1[k,2]~lbnds2[k,2];
    prt = printfm(temp, mask, fmt);?;
  endfor;
  ?;
  "Double Regression";
